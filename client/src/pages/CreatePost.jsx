...
const CreatePost = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      setLoading(true);
      try {
        const response = await fetch("https://dall-e-fh8h.onrender.com/api/v1/post", {
          method: 'POST',
          headers: {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch('https://dall-e-fh8h.onrender.com/api/v1/dalle', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
  ...
  return (
    <section className='max-w-7xl mx-auto'>
      ...
      focus:border-blue-500 w-64 p-3 h-64 flex justify-center items-center'>
            {
              form.photo ? (
                <img src={form.photo} alt={form.prompt} className='w-full h-full object-contain opacity-100' />
              ) : (
                <img
                  src={preview}
                  alt='preview'
